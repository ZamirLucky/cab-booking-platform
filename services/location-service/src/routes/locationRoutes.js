...
router.get('/locations/:id/weather', requireAuth, async (req, res, next) => {
  try {
    ...
    try {
      ...
      const apiError = weatherErr.response?.data?.error;

      console.error('[locationRoutes] External WeatherAPI.com error:', {
        status: weatherErr.response?.status,
        code: apiError?.code,
        message: apiError?.message || weatherErr.message,
        url: weatherErr.config?.url,
        params: {
          ...weatherErr.config?.params,
          key: '[redacted]'
        }
      });
